...
let ray_sphere = (function () {
  ...
  return function (ray, sphere, outPt) {
    ...
    let aLength = vec3.dot(e, d) / vec3.length(d); //Projection formula: dot(a, b) / |b|
...
let ray_box = (function () {
  ...
  return function (ray, box, outPt) {
    ...
    let f = [vec3.dot(X, d), vec3.dot(Y, d), vec3.dot(Z, d)];//The cos values of the ray on the X, Y, Z
    let e = [vec3.dot(X, p), vec3.dot(Y, p), vec3.dot(Z, p)];//The projection length of P on X, Y, Z
